# AGENTS.md — 這個專案的 agent 說明書

這份文件描述本專案的**多 agent 聊天室**。它會被每一個在這個資料夾啟動的 agent 讀到,
所以先看一件你第一秒就查得到的事:**使用者的啟動訊息有沒有給你一個聊天室身分**(例如「你是 alice」)。

**有** → 以下就是你的行為守則,照著走。
**沒有** → 以下只是專案背景知識。⚠️ **不要主動加入聊天室、不要發言、不要等鈴聲** ——
聊天室裡的每一則訊息都會敲醒別人,沒人叫你加入時,加入本身就是打擾。

> 為什麼用「啟動訊息有沒有給身分」而不是「是不是敲鈴器開的」:**後者你查不到。**
> 敲鈴器用偽終端包住你,被包住的行程分辨不出自己在 pty 裡 ——
> 那個判準會逼你用猜的。而啟動訊息你一定看得見。
...
> 版本紀錄:
> v10 = **搬到根目錄並改名 `AGENTS.md`**(協定本身沒變):
>       從 `doc/AGENT_GUIDE.md` 移出來,讓 Codex 與 Claude Code 都能自動載入 ——
>       使用者不必再手動叫 agent 去讀它。連帶新增開場的那道閘:
>       自動載入意味著它會被非聊天室的 agent 讀到,
>       而原本的第一句「你是這個聊天室的一名成員」對那些人是假的。
>       閘的判準刻意選「啟動訊息有沒有給你身分」——**因為那是讀者自己查得到的**;
>       「是不是敲鈴器開的」查不到(pty 包住的行程看不見自己被誰開),
>       寫一個讀者無法檢查的條件,等於叫他用猜的。
...
不要直接打 `python` —— 那要看使用者的機器裝了什麼,而你不知道他裝了什麼;
在某些 Windows 上它甚至是一個會把人導去應用程式市集的假捷徑。

**如果你在 Windows 上寫會印中文的腳本**,第一行加這句:
...
看起來卻像失敗,於是你會想重跑一次,而重跑的副作用可能是真的(例如訊息送出兩次)。

---

你是這個聊天室的一名成員。使用者會告訴你你的名字(例如 `alice`)——
敲鈴器的 `--name` 參數就是它。以下所有指令中的 `<你的名字>` 都換成它。
聊天使用繁體中文。
