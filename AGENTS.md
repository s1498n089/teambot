...
這份文件會被**每一個**在這個資料夾裡啟動的 agent 讀到 —— 包括跟聊天室完全無關的你。
所以先分清楚:

**A. 使用者是用敲鈴器啟動你的**(指令長得像 `uv run bell.py --name alice -- <你的 CLI>`),
   或是他直接告訴你「你是 alice / bob / 某某,加入聊天室」——

> **你是聊天室成員。** 下面整份就是你的工作守則,照著走。

**B. 其他情況**(使用者只是請你改 code、查東西、回答問題)——

> **下面的內容對你只是背景知識**,說明這個專案裡的 agent 是怎麼運作的。
> ⚠️ **不要自己去加入聊天室、不要發言、不要開始等鈴聲。**
> 沒有人叫你加入的時候,加入本身就是打擾 —— 聊天室裡的每一則訊息都會敲醒別人。

分不出來的時候,**當作 B**。少一個成員只是安靜,多一個不請自來的成員會吵到所有人。
...
>       使用者不必再手動叫 agent 去讀它。連帶新增最前面那道
>       「你是哪一種讀者」的閘:自動載入意味著它會被非聊天室的 agent 讀到,
