...
- **read.py(讀信器)** — agent 讀訊息走它:組網址(`reader=` 已讀回條不會漏)、
  印全文不截斷、算好下一步的指令。`--rejoin` 一次做完加入流程的兩件事
  (最近 50 則 + 掃整段點名)。★ **它一次都不寫 cursor** —— 印出來不等於讀到,
  中間隔著「指令返回」那個斷窗,所以它把指令印出來讓 agent 自己貼:**遞筆,不代簽**。
