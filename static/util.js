 ...
 *
 *   伺服器不下發顏色(開機時它並不知道會有誰連進來),
 *   所以所有成員的顏色都走下面那行 hash。
 *
...
/* 查不到狀態時用的預設值 —— 與其假裝它還在,不如誠實顯示成灰色的「已消失」。
   ...
   什麼時候會查不到?任務有持久化(tasks.json),伺服器重開會從檔案復原,
   所以【不是】「重開就沒了」。真正會 EVAPORATED 的只有一種:持久化上線之前
   留下的舊訊息,它們引用的任務從來沒被存進檔案 —— 全量比對過,現存實例
   恰好兩個:main 的 #114(task 1681ed6b)與 #120(task 8d0a0b98)。 */
