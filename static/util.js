...
/* 查不到狀態時用的預設值 —— 與其假裝它還在,不如誠實顯示成灰色的「已消失」。
   ...
   今天還會 EVAPORATED 的【只有一種】:持久化上線之前留下的舊訊息,
   它們引用的任務從來沒被存進檔案。全量比對過,現存實例恰好兩個 ——
   main 的 #114(task 1681ed6b)與 #120(task 8d0a0b98)。

   (原本這裡還寫了「清理測試房間時一起刪掉的那批」—— 那是推論,而且是錯的:
    清房間時訊息跟任務是一起刪的,所以「訊息還在、任務不在」對它們不可能發生。) */
