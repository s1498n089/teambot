...
function createApi(notify) {
  ...
  return {
    ...
    /**
     * 這個房間的**成員**:有 cursor 的人。
     *
     * ★ 跟 `presence()` 是兩份不同的資料,不要混:
     *
     *     成員(這支)   有 cursor —— 持久,關掉視窗也還在
     *     在場(presence) 連線開著 —— 短暫,關掉視窗就消失
     *
     *   邀請按鈕要看的是**成員**:剛被邀請的 agent 最多要 30 秒才會連上來,
     *   這段期間它已經是成員、卻還不在場。拿在場去判斷的話,
     *   那 30 秒裡按鈕會一直顯示「可邀請」,使用者會重複按。
     */
    cursors: function (room) {
      return request(`/api/rooms/${encodeURIComponent(room)}/cursors`, undefined, true);
    },

    /**
     * 邀請某人進這個房 —— **就是替他建一個 cursor**,沒有別的動作。
     *
     * 建在 0:代表「一則都還沒讀」,所以他進來之後會看得到房間的歷史。
     *
     * ★ 這支**不檢查對方是不是已經是成員**,而伺服器那邊 cursor 倒退是允許的
     *   (那條規則是為了「我想重讀一段」而留的)。所以對一個【已經是成員】的人
     *   呼叫它,會把他的進度打回 0 —— 後果是他重讀整個房間。
     *
     *   不在這裡擋的理由:呼叫端本來就只對「不是成員的人」顯示按鈕,
     *   而萬一真的撞上(名單過時個幾秒),後果是**多讀**不是漏讀 ——
     *   那是這個專案一貫接受的那一側(見 AGENTS.md:落後無害、超前有害)。
     *   要為它加一道鎖之前,先確認擋掉的東西比「重讀一次」更貴。
     */
    invite: function (room, name) {
      return request(`/api/rooms/${encodeURIComponent(room)}`
                     + `/cursor/${encodeURIComponent(name)}?last_id=0`,
                     { method: "PUT" });
    },
