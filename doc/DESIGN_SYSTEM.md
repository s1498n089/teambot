...
### ⚠️ 待 allen 確認:視覺簽名的落差

下面這些是**需求書明確要過、而實作沒有做到或後來拿掉**的。
它們不是死碼(死碼不用問),是**需求方點名要的東西** —— 所以列在這裡等他決定,
沒有主動改回來。

| 需求書寫的 | 現在的實際狀況 | 要問的 |
|---|---|---|
| **Scanlines**(§1 視覺簽名、§2 紋理、開頭「推翻舊共識」的理由,三次點名) | **已停用**。`styles.css` 留著一行註記:「scanlines 已停用;要復原,把當時的 body::after 規則加回來即可」 | **要復原嗎?** |
| **Chromatic aberration**(RGB 分離,列為第一號視覺簽名) | 只在標題 `.hdr-title` 有一處**靜態**色差,不是全域效果 | 接受現況,還是擴大? |
| **字型**:Headings `Orbitron`、Body `JetBrains Mono` | **兩個都沒有載入**。`index.html` 只載 `Share Tech Mono`(用在 metadata 與控制項),中文本文走系統無襯線 | 要補嗎?(中文本文套 mono 的老問題也在這裡) |
| **`accentSecondary: #ff00ff`**(hot magenta,列為三大 accent 之一) | 只用在標題色差的一半;`--magenta` 變數 2026-07-27 因零使用被刪 | 要讓 magenta 回到主力位置嗎? |
| **按鈕變體**:secondary / outline / ghost / glitch 四種 | 變體沒做;glitch 只保留為動畫(白名單三處) | 接受現況? |
