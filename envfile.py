...
## 為什麼換成 python-dotenv(2026-07-28)

這裡原本是一個自己寫的 .env 解析器,理由是「格式自己寫、協定用庫」,
而且它自己寫下了退場條件:「哪天真的需要引號跳脫、多行值、${VAR} 展開,
那就是該換 python-dotenv 的時候。」

**真正讓它退場的不是那個條件,是另一個更根本的理由:**

    「能不要自己寫就不要自己寫。裝一個套件對開發者無感,
      重點是現成好用就好,而且查資源超方便。」

自己寫的那份確實只有十幾行,但它的成本不在行數:

  - 它只支援 `KEY=value`。引號、`export` 前綴、`${VAR}` 展開一律不支援 ——
    而使用者不會知道,他只會發現「我寫的設定沒生效」。
  - 遇到問題時,搜尋「python dotenv 怎麼寫引號」有一百篇答案;
    搜尋「這個專案的 envfile.py 怎麼寫引號」有零篇。

★ 值得記的是:**當初那個退場條件寫得太窄了。** 它只想到「功能不夠用時才換」,
  沒想到「查得到答案」本身就是一種功能 —— 而那是自己寫的東西永遠給不了的。

換過來之後,上面那三種寫法全部自動支援(實測過)。
